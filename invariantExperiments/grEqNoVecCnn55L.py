...
def layer(inpt, k_h, k_w, c_i, c_o, s_h, s_w, group=True, padding="SAME", first=True, useType="test", resuse_batch_norm=False, normalize="bn", wd=0.0001):
	...
	if group:
		...
		weightMask = tf.contrib.image.rotate(weightMask, numpy.pi/4)
		weightMask = tf.transpose(weightMask, [1,2,3,0])
	conv_in = cc.layers.convAllRotations(inpt, [k_h, k_w, c_i, c_o], k_h, s_h, s_w, first=first, padding=padding, num_bins=num_bins, weightMask=weightMask, steerable=True)
...
def inference(images, batch_size, useType="test", first=True, resuse_batch_norm=False, fs=3, normalizationMode="bn"):
	...
	with tf.variable_scope("conv4_1", reuse=(not first)) as scope:
		...
		conv4_1 = tf.reduce_max(tf.reshape(conv4_1_relu, [batch_size,1,1,96,16]), axis=-1)
