...
def opLayer(inpt, k_h, k_w, c_i, c_o, s_h, s_w, padding="SAME", first=True, useType="test", resuse_batch_norm=False, normalize="bn", wd=0.0001):
	convW, convb = cc.misc.getWeightsNBiases(first, k_h, k_w, c_o, 2*c_i, s_h, s_w, wd=wd, lossType="nl")
	weightMask = tf.ones([c_o, k_h, k_w, 2*c_i], tf.float32)
	weightMask = tf.contrib.image.rotate(weightMask, numpy.pi/4)
	weightMask = tf.transpose(weightMask, [1,2,3,0])
	conv_in, angles = cc.layers.rotInvarianceWithArgMax(inpt, convW, convb, s_h, s_w, padding=padding, num_bins=16, weightMask=weightMask)
...
def inference(images, batch_size, useType="test", first=True, resuse_batch_norm=False, fs=3, normalizationMode="bn"):
	...
	with tf.variable_scope("conv4_1", reuse=(not first)) as scope:
		...
		conv4_1_relu, angles_4_1 = opLayer(conv3_2, k_h, k_w, c_i, c_o, s_h, s_w, padding="VALID", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize=False, wd=wd)
