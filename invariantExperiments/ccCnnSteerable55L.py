...
@tf.custom_gradient
def weightToThetaGradsApprox(inpt, output, mask, weights, qThetas, thetas):
	def grad(dy):
...
@tf.custom_gradient
def weightToThetaGrads(inpt, output, mask, weights, qThetas, thetas, weightMask, steerableCoefs):
	def grad(dy):
		weightShape = weights.get_shape().as_list()
		numAngels = weightShape[0]
		num_bins = weightShape[0]-1
		offset = num_bins//(2*cc.misc.num_angles)
		...
		verTWeights = cc.transformations.rotateVectors(tWeights, [numpy.pi/2])
		...
		# utWeights = tf.gather(verWeights, qThetas)
		# npReduction = numpy.zeros([weightShape[-1]]+weightShape[1:], dtype=numpy.float32)
		# for i in range(weightShape[-1]):
		# 	npReduction[i,:,:,:,i] = 1
		# utWeights = tf.reduce_sum(tf.multiply(utWeights, npReduction), axis=-1)
		exInpt = tf.expand_dims(inpt, axis=1)
		exInpt = tf.expand_dims(exInpt, axis=1)
		exInpt = tf.expand_dims(exInpt, axis=1)
		# vecRotThetaGrads = tf.reduce_sum(tf.multiply(exInpt, utWeights), axis=[-3,-2,-1])
		# steerableCoefs = tf.get_collection("weights")[-1]
		gradientKernels = cc.misc.getSteerableGradientKernel(weightShape[1], npAngles, weightShape[3]//2, weightShape[4], steerableCoefs, weightMask)
		...
		thetaGrads = tf.reduce_sum(tf.multiply(exInpt, woa), axis=[-3,-2,-1])*dy*tf.cast(mask, tf.float32)
		return (None, dy, None, None, None, thetaGrads, None, None)
	return tf.identity(output), grad

@tf.custom_gradient
def weightToThetaGradsNoMask(inpt, output, mask, weights, qThetas, thetas, steerableCoefs):
	def grad(dy):
		weightShape = weights.get_shape().as_list()
		numAngels = weightShape[0]
		num_bins = weightShape[0]-1
		...
		# vecRotThetaGrads = tf.reduce_sum(tf.multiply(exInpt, utWeights), axis=[-3,-2,-1])
		# steerableCoefs = tf.get_collection("weights")[-1]
		gradientKernels = cc.misc.getSteerableGradientKernel(weightShape[1], npAngles, weightShape[3]//2, weightShape[4], steerableCoefs, None)
		allWoa = verWeights + gradientKernels
		allWoaL = tf.unstack(allWoa, axis=-1)
		indL = tf.unstack(qThetas, axis=-1)
		woa = []
...
def layer(inpt, k_h, k_w, c_i, c_o, s_h, s_w, padding="SAME", first=True, useType="test", resuse_batch_norm=False, normalize="vn", wd=0.0001):
	...
	weightMask = None
	conv_in, angles = cc.layers.conv(inpt, k_h, None, c_i, c_o, s_h, s_w, first=first, useType=useType, padding=padding, num_bins=16, normalize=normalize, count=(useType=="train"), weightMask=weightMask, steerable=True)
...
def inference(images, batch_size, useType="test", first=True, resuse_batch_norm=False, fs=3, normalizationMode="bn"):
	...
	with tf.variable_scope("conv2_1", reuse=(not first)) as scope:
		...
		# 	maxPool2_1, pool2_1Indexes = tf.nn.max_pool_with_argmax(conv2_1_relu, ksize=[1, k_h, k_w, 1], strides=[1, s_h, s_w, 1], padding=padding, name="maxPool")
		# 	intIndeces = tf.cast(pool2_1Indexes, tf.int32)
		# 	pooled_angles_2_1 = cc.ops.poolByIndex(angles_2_1, intIndeces)
	...
	with tf.variable_scope("conv3_1", reuse=(not first)) as scope:
		...
		# with tf.variable_scope("maxPool") as scope:
		# 	k_h = 2; k_w = 2; s_h = 2; s_w = 2; padding = 'VALID'
		# 	maxPool3_1, pool3_1Indexes = tf.nn.max_pool_with_argmax(conv3_1_relu, ksize=[1, k_h, k_w, 1], strides=[1, s_h, s_w, 1], padding=padding, name="maxPool")
		# 	intIndeces = tf.cast(pool3_1Indexes, tf.int32)
		# 	pooled_angles_3_1 = cc.ops.poolByIndex(angles_3_1, intIndeces)
	...
	with tf.variable_scope("conv3_1.5", reuse=(not first)) as scope:
		...
		shape = conv3_15.get_shape()
		conv3_15 = cc.layers.normalizeVectorField(conv3_15, shape[-3].value, shape[-3].value)
	...
	with tf.variable_scope("conv4_1", reuse=(not first)) as scope:
		...
		conv4_1W, conv4_1b = getWeightsNBiases(first, 1, 1, c_o, 2*c_i, s_h, s_w, wd=0, mode="nl")
		# weightMask = tf.ones([c_o, k_h, k_w, 2*c_i], tf.float32)
		# weightMask = tf.contrib.image.rotate(weightMask, numpy.pi/4)
		# weightMask = tf.transpose(weightMask, [1,2,3,0])
		weightMask = None
		...
		for angle_index, angle in enumerate(npAngles):
			...
			with tf.variable_scope("weights", reuse=reuse) as scope:
				rotatedWeightsL.append(cc.misc.getSteerableKernel(k_h, angle, c_i, c_o, first=(not reuse)))
				# rotatedWeightsL.append(cc.misc.getSteerableKernel(k_h, angle, c_i, c_o, first=(not reuse))*weightMask)
		...
		for angle in range(0, num_bins, num_bins//num_angles):
			...
			# 		c42Conv2 = cc.layers.conv_2tf(conv3_2, weightSet, c_i, c_o, 1, 1, "VALID", monitor=True)
			# else:
		...
		steerableCoefs = tf.get_collection("weights")[-1]
		if weightMask != None:
			convRes = weightToThetaGrads(conv3_2, convRes, convMask, rotatedWeights, quantized, thetas2, steerableCoefs, weightMask)
		else:
			convRes = weightToThetaGradsNoMask(conv3_2, convRes, convMask, rotatedWeights, quantized, thetas2, steerableCoefs)
