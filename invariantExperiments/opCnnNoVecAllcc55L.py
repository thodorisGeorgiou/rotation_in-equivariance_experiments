...
def layer(inpt, k_h, k_w, c_i, c_o, s_h, s_w, padding="SAME", first=True, useType="test", resuse_batch_norm=False, normalize="bn", wd=0.0001):
	...
	conv_in = cc.layers.rotInvarianceWithMax(inpt, convW, convb, s_h, s_w, padding=padding, num_bins=16, weightMask=weightMask)
	# conv_in = cc.layers.rotInvarianceWithSum(inpt, convW, convb, s_h, s_w, padding=padding, num_bins=16, weightMask=weightMask)
...
def inference(images, batch_size, useType="test", first=True, resuse_batch_norm=False, fs=3, normalizationMode="bn"):
	...
	with tf.variable_scope("conv1_1", reuse=(not first)) as scope:
		k_h = 9; k_w = 9; c_i = 2; c_o = 16; s_h = 1; s_w = 1
		conv1_1_relu = layer(images, k_h, k_w, c_i, c_o, s_h, s_w, padding="SAME", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize=False, wd=wd)
	...
	with tf.variable_scope("conv4_1", reuse=(not first)) as scope:
		...
		conv4_1_relu = layer(conv3_2, k_h, k_w, c_i, c_o, s_h, s_w, padding="VALID", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize=False, wd=wd)
