...
def layer(inpt, k_h, k_w, c_i, c_o, s_h, s_w, group=True, padding="SAME", first=True, useType="test", resuse_batch_norm=False, normalize="bn", wd=0.0001, steerable=True, cartesian=False, clifford=False):
	...
	if cartesian:
		...
		if k_h == 1:
			...
			conv_in, angles = cc.layers.cliffordConvAllRotationsK(inpt, [k_h, k_w, c_i, c_o], k_h, s_h, s_w, first=first, padding=padding, num_bins=num_bins, weightMask=weightMask, steerable=steerable)
...
def inference(images, batch_size, useType="test", first=True, resuse_batch_norm=False, fs=3, normalizationMode="bn"):
	...
	with tf.variable_scope("conv3_1.5", reuse=(not first)) as scope:
		...
		conv3_15 = layer(conv3_1, k_h, k_w, c_i, c_o, s_h, s_w, padding="SAME", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize=normalizationMode, wd=wd, cartesian=True)
	...
	with tf.variable_scope("conv4_1", reuse=(not first)) as scope:
		...
		conv4_1 = layer(conv3_2, k_h, k_w, c_i, c_o, s_h, s_w, padding="VALID", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize=normalizationMode, wd=wd, steerable=False, clifford=True)
...
# out, out_Angles = layer(fc2_relu, k_h, k_w, c_i, c_o, s_h, s_w, padding="VALID", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize=False, wd=wd, steerable=False, clifford=True)
	...
	# 	fc1, fc1_angles = layer(conv4_1, k_h, k_w, c_i, c_o, s_h, s_w, padding="VALID", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize=normalizationMode, wd=wd, steerable=False, clifford=True)
	# 	if useType=="train":
	# 		fc1 = tf.nn.dropout(fc1, rate=0.3)
	#fc1
	with tf.variable_scope("fc1", reuse=(not first)) as scope:
		fc1W, fc1b = getFCWeightsNBiases(first, 96, 96, wd=1e-9)
	...
	with tf.variable_scope("fc2", reuse=(not first)) as scope:
		fc2W, fc2b = getFCWeightsNBiases(first, 96, 96, wd=1e-9)
		fc2 = tf.matmul(tf.squeeze(fc1), fc2W) + fc2b
	...
	with tf.variable_scope("out", reuse=(not first)) as scope:
		outW, outb = getFCWeightsNBiases(first, 10, 96, wd=1e-9)
