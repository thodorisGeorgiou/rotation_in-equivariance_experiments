...
def layer(inpt, k_h, k_w, c_i, c_o, s_h, s_w, group=True, padding="SAME", first=True, useType="test", resuse_batch_norm=False, normalize="bn", wd=0.0001, steerable=True, cartesian=False, clifford=False):
	# convW, convb = getWeightsNBiases(first, k_h, k_w, c_o, c_i, s_h, s_w, wd=wd, mode="nl")
	num_bins = 16
	if group and not cartesian:
	...
	if cartesian:
		if clifford:
			exit("Can't have both cartesian and clifford.")
		conv_in, mask = cc.layers.convAllRotationsCartesianOut(inpt, [k_h, k_w, c_i, c_o], k_h, s_h, s_w, first=first, padding=padding, num_bins=num_bins, weightMask=weightMask, steerable=steerable, cartesian=cartesian)
	elif clifford:
		conv_in, angles = cc.layers.cliffordConvAllRotations(inpt, [k_h, k_w, c_i, c_o], k_h, s_h, s_w, first=first, padding=padding, num_bins=num_bins, weightMask=weightMask, steerable=steerable)
	...
	if normalize == "bn":
		if cartesian:
			...
			# tf.add_to_collection("beforeBatchNorm", conv_in)
			conv_in = cc.layers.batch_norm_only_rescale_learn_scale_cartesian(conv_in, c_o, useType, reuse=resuse_batch_norm)
			# tf.add_to_collection("afterBatchNorm", conv_in)
			# exit()
			conv_in = cc.layers.normalizeVectorField(conv_in, 1, 1)
			# tf.add_to_collection("afterVecNorm", conv_in)
		else:
			conv_in = cc.layers.batch_norm(conv_in, useType, reuse=resuse_batch_norm)
			conv_in = tf.nn.relu(conv_in)
...
def inference(images, batch_size, useType="test", first=True, resuse_batch_norm=False, fs=3, normalizationMode="bn"):
	...
	with tf.variable_scope("conv1_1", reuse=(not first)) as scope:
		...
		conv1_1_relu = layer(images, k_h, k_w, c_i, c_o, s_h, s_w, group=False, padding="SAME", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize=normalizationMode, wd=wd)
	...
	with tf.variable_scope("conv2_1", reuse=(not first)) as scope:
		...
		conv2_1_relu = layer(conv1, k_h, k_w, c_i, c_o, s_h, s_w, padding="SAME", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize=normalizationMode, wd=wd)
	...
	with tf.variable_scope("conv2_2", reuse=(not first)) as scope:
		...
		conv2_2 = layer(conv2_1, k_h, k_w, c_i, c_o, s_h, s_w, padding="SAME", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize=normalizationMode, wd=wd)
	...
	with tf.variable_scope("conv3_1", reuse=(not first)) as scope:
		...
		conv3_1_relu = layer(conv2_2, k_h, k_w, c_i, c_o, s_h, s_w, padding="SAME", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize=normalizationMode, wd=wd)
	...
	with tf.variable_scope("conv3_1.5", reuse=(not first)) as scope:
		...
		conv3_15 = layer(conv3_1, k_h, k_w, c_i, c_o, s_h, s_w, padding="SAME", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize=normalizationMode, wd=wd)
	...
	with tf.variable_scope("conv4_1", reuse=(not first)) as scope:
		...
		conv4_1 = layer(conv3_2, k_h, k_w, c_i, c_o, s_h, s_w, padding="VALID", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize=normalizationMode, wd=wd)
	...
	with tf.variable_scope("fc1", reuse=(not first)) as scope:
		...
		fc1 = layer(conv4_1, k_h, k_w, c_i, c_o, s_h, s_w, padding="VALID", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize=normalizationMode, wd=wd, steerable=False)
		tf.add_to_collection("activationMagnitudes", conv4_1)
		# if useType=="train":
	...
	with tf.variable_scope("fc2", reuse=(not first)) as scope:
		k_h = 1; k_w = 1; c_i = 96; c_o = 96; s_h = 1; s_w = 1
		fc2_relu = layer(fc1, k_h, k_w, c_i, c_o, s_h, s_w, padding="VALID", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize=normalizationMode, wd=wd, steerable=False, cartesian=True)
		# fc2_relu = tf.reshape(fc2_relu, [batch_size,1,16,96])
		# angles = tf.constant([a*2*numpy.pi/16 for a in range(16)])
		# angles = tf.expand_dims(tf.expand_dims(tf.expand_dims(angles, axis=-1), axis=0), axis=0)
		# angles = tf.tile(angles, [batch_size, 1, 1, 96])
		# fc2_cart = cc.transformations.changeToCartesian(fc2_relu, angles)
		# fc2_cart = tf.reduce_sum(fc2_cart, axis=-2, keepdims=True) 
		# # fc2, fc2_angles = cc.transformations.changeToPolar(fc2_cart, c_o)
		# fc2_cart = cc.layers.batch_norm_only_rescale_learn_scale_cartesian(fc2_cart, c_o, useType, reuse=(not first))
		fc2Shape = fc2_relu.get_shape().as_list()
		# if useType=="train":
		# 	npmask = numpy.ones(fc2Shape[:-1]+[fc2Shape[-1]//2,1], dtype=numpy.float32)
		# 	pmask = tf.constant(npmask)
		# 	dmask = tf.nn.dropout(pmask, rate=0.5)
		# 	mask = tf.reshape(tf.tile(dmask, [1,1,1,1,2]), fc2Shape)
		# 	fc2_relu = fc2_relu*mask
	#out
	with tf.variable_scope("out", reuse=(not first)) as scope:
		k_h = 1; k_w = 1; c_i = 96; c_o = 10; s_h = 1; s_w = 1; num_bins = 16
		out, out_Angles = layer(fc2_relu, k_h, k_w, c_i, c_o, s_h, s_w, padding="VALID", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize=False, wd=wd, steerable=False, clifford=True)
