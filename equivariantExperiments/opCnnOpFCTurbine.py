...
def layer(inpt, k_h, k_w, c_i, c_o, s_h, s_w, padding="SAME", first=True, useType="test", resuse_batch_norm=False, normalize="bn", wd=0.0001, fc=False):
	...
	conv_in, angles = cc.layers.rotInvarianceWithArgMax(inpt, convW, convb, s_h, s_w, padding="SAME", num_bins=16, weightMask=weightMask)
	if padding == "VALID":
		weightsShape = convW.get_shape().as_list()
		inptShape = inpt.get_shape().as_list()
		k_w = weightsShape[0]
		k_h = weightsShape[1]
		i_w = inptShape[1]
		i_h = inptShape[2]
		conv_in = tf.image.crop_to_bounding_box(conv_in, numpy.floor(k_w/2).astype(numpy.int32), numpy.floor(k_h/2).astype(numpy.int32), i_w-2*numpy.floor(k_w/2).astype(numpy.int32), i_h-2*numpy.floor(k_h/2).astype(numpy.int32))
		angles = tf.image.crop_to_bounding_box(angles, numpy.floor(k_w/2).astype(numpy.int32), numpy.floor(k_h/2).astype(numpy.int32), i_w-2*numpy.floor(k_w/2).astype(numpy.int32), i_h-2*numpy.floor(k_h/2).astype(numpy.int32))
	if normalize == "bn":
...
def inference(images, batch_size, useType="test", first=True, resuse_batch_norm=False, fs=3, normalizationMode="bn"):
	if useType=="train":
		wd = tf.Variable(0.1, dtype=tf.float32, trainable=False)
	else:
		wd = 0.0001
	#conv1_1
	with tf.variable_scope("conv1_1", reuse=(not first)) as scope:
		k_h = 9; k_w = 9; c_i = 1; c_o = 16; s_h = 1; s_w = 1
		conv1_1_relu, angles_1_1 = layer(images, k_h, k_w, c_i, c_o, s_h, s_w, padding="SAME", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize="bn", wd=wd)
	...
	with tf.variable_scope("conv2_1", reuse=(not first)) as scope:
		...
		conv2_1_relu, angles_2_1 = layer(conv1, k_h, k_w, c_i, c_o, s_h, s_w, padding="SAME", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize="bn", wd=wd)
	...
	with tf.variable_scope("conv2_2", reuse=(not first)) as scope:
		...
		conv2_2_relu, angles_2_2 = layer(conv2_1, k_h, k_w, c_i, c_o, s_h, s_w, padding="SAME", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize="bn", wd=wd)
	...
	with tf.variable_scope("conv3_1", reuse=(not first)) as scope:
		...
		conv3_1_relu, angles_3_1 = layer(conv2_2, k_h, k_w, c_i, c_o, s_h, s_w, padding="SAME", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize="bn", wd=wd)
	...
	with tf.variable_scope("conv3_1.5", reuse=(not first)) as scope:
		...
		conv3_15_relu, angles_3_15 = layer(conv3_1, k_h, k_w, c_i, c_o, s_h, s_w, padding="SAME", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize="bn", wd=wd)
	...
	# 	conv4_1_relu, angles_4_1 = layer(conv3_2, k_h, k_w, c_i, c_o, s_h, s_w, padding="SAME", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize="bn", wd=wd)
	...
	with tf.variable_scope("conv4_1", reuse=(not first)) as scope:
		...
		conv4_1_relu, angles_4_1 = layer(conv3_2, k_h, k_w, c_i, c_o, s_h, s_w, padding="VALID", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize="bn", wd=wd)
		# conv4_1 = cc.layers.batch_norm_only_rescale_learn_scale(conv4_1_relu, useType, resuse_batch_norm)
		conv4_1 = conv4_1_relu
		if useType=="train":
			conv4_1 = tf.nn.dropout(conv4_1, rate=0.2)
		conv4_1_cart = cc.transformations.changeToCartesian(conv4_1, angles_4_1, False)
		# conv4_1 = convRes
		...
		tf.add_to_collection("activationAngles", angles_4_1)
	...
	with tf.variable_scope("fc1", reuse=(not first)) as scope:
		...
		fc1_relu, angles_fc1 = layer(conv4_1_cart, k_h, k_w, c_i, c_o, s_h, s_w, padding="VALID", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize="bn", wd=wd, fc=True)
		# conv4_1 = cc.layers.batch_norm_only_rescale_learn_scale(conv4_1_relu, useType, resuse_batch_norm)
		fc1 = fc1_relu
		if useType=="train":
			fc1 = tf.nn.dropout(fc1, rate=0.2)
		fc1_cart = cc.transformations.changeToCartesian(fc1, angles_fc1, False)
		tf.add_to_collection("activationMagnitudes", fc1)
		tf.add_to_collection("cartesianRepresentationPooled", fc1_cart)
		tf.add_to_collection("activationAngles", angles_fc1)
	#fc2
	with tf.variable_scope("fc2", reuse=(not first)) as scope:
		k_h = 1; k_w = 1; c_i = 96; c_o = 96; s_h = 1; s_w = 1
		# fc2_relu, angles_fc2 = layer(fc1_cart, k_h, k_w, c_i, c_o, s_h, s_w, padding="VALID", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize="bn", wd=wd, fc=True)
		# # conv4_1 = cc.layers.batch_norm_only_rescale_learn_scale(conv4_1_relu, useType, resuse_batch_norm)
		# fc2 = fc2_relu
		# if useType=="train":
		# 	fc2 = tf.nn.dropout(fc2, rate=0.2)
		# fc2_cart = cc.transformations.changeToCartesian(fc2, angles_fc2, False)
		# tf.add_to_collection("activationMagnitudes", fc2)
		# tf.add_to_collection("cartesianRepresentationPooled", fc2_cart)
		# tf.add_to_collection("activationAngles", angles_fc2)
		num_bins = 16
		fc2W, fc2b = cc.misc.getWeightsNBiases(first, k_h, k_w, c_o, 2*c_i, s_h, s_w, wd=wd, lossType="wd")
		fc2_relu = cc.layers.whatShouldBeConvAllRotations(fc1_cart, fc2W, fc2b, s_h, s_w, padding="VALID", num_bins=num_bins, weightMask=None)
		fc2_relu = cc.layers.batch_norm_only_rescale_learn_scale(fc2_relu, useType, resuse_batch_norm)
	...
	with tf.variable_scope("out", reuse=(not first)) as scope:
		...
		out_relu, angles_out = layer(fc2_cart, k_h, k_w, c_i, c_o, s_h, s_w, padding="VALID", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize=False, wd=wd, fc=True)
		# conv4_1 = cc.layers.batch_norm_only_rescale_learn_scale(conv4_1_relu, useType, resuse_batch_norm)
		out = out_relu
		prob = tf.nn.softmax(out, name="ClassProbabilities")
	#outAngles
	with tf.variable_scope("outAngles", reuse=(not first)) as scope:
		angles = [a*2*numpy.pi/16 for a in range(16)]
		nhfcW = numpy.array([[numpy.cos(an), numpy.sin(an)] for an in angles], dtype=numpy.float32)
		hfcW = tf.constant(nhfcW)
		fc2L = tf.unstack(tf.squeeze(tf.reshape(fc2_relu, [batch_size, 1, 1, 16, 96])), axis=-1)
		resL = []
		for fc2Node in fc2L:
			resL.append(tf.matmul(fc2Node, hfcW))
		hfc = tf.stack(resL, axis=-1)
		hfc = tf.reshape(hfc, [batch_size, 2*96])
		angleW, angleB = getFCWeightsNBiases(first, 20, 192, wd=wd)
		softmax_linear = tf.add(tf.matmul(hfc, angleW), angleB, name=scope.name)
		tanh = tf.math.tanh(softmax_linear)
		tanh = tf.reshape(tanh, tanh.get_shape().as_list()[:-1]+[10,2])
		norm = tf.norm(tanh, axis=-1, keepdims=True)
		normTanh = tanh/norm
		outSin, outCos = tf.unstack(normTanh, axis=-1)
		outAngle = tf.expand_dims(tf.expand_dims(tf.atan2(outSin, outCos), axis=1), axis=1)
	return tf.squeeze(out), tf.squeeze(prob), angles_out, outAngle
	# #outAngles
	# with tf.variable_scope("outAngles", reuse=(not first)) as scope:
	# 	angleW, angleB = getFCWeightsNBiases(first, 20, 192, wd=wd)
	# 	softmax_linear = tf.add(tf.matmul(tf.squeeze(fc2_cart), angleW), angleB, name=scope.name)
	# 	tanh = tf.math.tanh(softmax_linear)
	# 	tanh = tf.reshape(tanh, tanh.get_shape().as_list()[:-1]+[10,2])
	# 	norm = tf.norm(tanh, axis=-1, keepdims=True)
	# 	normTanh = tanh/norm
	# 	outSin, outCos = tf.unstack(normTanh, axis=-1)
	# 	outAngle = tf.atan2(outSin, outCos)
	# return tf.squeeze(out), tf.squeeze(prob), angles_out, tf.expand_dims(outAngle, axis=1)
	# return tf.squeeze(out), tf.squeeze(prob), angles_out, tf.expand_dims(tf.expand_dims(outAngle, axis=1), axis=1)
