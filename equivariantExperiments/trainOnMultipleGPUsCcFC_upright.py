...
# baseDir = "trained_angles/ccCcFCNet5L3x96_64_2x36_32_16_fs_9_all_7_16Bins_weightMask_trainAll_div2AnglesSupervision_dropout_0.2_0MAD_droppingLR_30_"
# baseDir = "trained_angles/ccCcFCNet5L3x96_64_2x36_32_16_fs_9_all_7_16Bins_weightMask_trainAll_dropout_0.2_0MAD_droppingLR_30_"+run+"_"
# baseDir = "trained_upright/ccCcFCNet5L3x96_64_2x36_32_16_fs_9_all_7_16Bins_weightMask_trainAll_dropout_3x0.5_0MAD_droppingLR_30_"+run+"_"
baseDir = "trained_upright_classification/grSumCc5L3x96_64_2x36_32_16_fs_9_all_7_16Bins_weightMask_trainAll_dropout_0.2_0MAD_droppingLR_30_"+run+"_"
...
if __name__ == '__main__':
	...
	# mnsitTrain = {'x': numpy.load("/tank/georgioutk/mnist/my_mnist_rot/mnist_rot_train_imanges.npy"), \
	# 'y': (numpy.load("/tank/georgioutk/mnist/my_mnist_rot/mnist_rot_train_labels.npy")[:,:1]).astype(numpy.int32), \
	# 'a': (numpy.load("/tank/georgioutk/mnist/my_mnist_rot/mnist_rot_train_labels.npy")[:,1:2])}
	...
	# preprocessing.gradientOrientationWithAngle(batch_size, mnsitTrain['x'], mnsitTrain['y'], mnsitTrain['a'], testBatch_size=testBatch_size, rotate=False)
	[next_example, next_label, next_angle], [next_testExample, next_testLabel, next_testAngle], [trainIterator, testIterator] = \
	preprocessing.colorWithRotation(batch_size, mnsitTrain['x'], mnsitTrain['y'], mnsitTrain['a'], testBatch_size=testBatch_size, rotate=False)
	...
	for i in range(numGpus):
		with tf.name_scope('tower_%d' % (i)) as scope:
			with tf.device('/gpu:%d' % i):
				...
				# softmax_linear, prob, unSupAngle, pred_angle = network.inference(nex[i], batch_size//numGpus, "train", first=(i==0), resuse_batch_norm=False, fs=7, normalizationMode="bn")
				...
				# pred_angle = tf.squeeze(cc.ops.reduceIndex(pred_angle, nla[i]))
	...
	for i in range(numGpusTest):
		with tf.name_scope('tower_%d' % (i)) as scope:
			with tf.device('/gpu:%d' % i):
				...
				# testSoftmax, testProb, testUnSupAngle, testPredAngle = network.inference(test_nex[i], testBatch_size//numGpusTest, "test", first=False, resuse_batch_norm=True, fs=7, normalizationMode="bn")
