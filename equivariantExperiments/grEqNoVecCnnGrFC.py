...
def inference(images, batch_size, useType="test", first=True, resuse_batch_norm=False, fs=3, normalizationMode="bn"):
	...
	with tf.variable_scope("fc2", reuse=(not first)) as scope:
		k_h = 1; k_w = 1; c_i = 96; c_o = 96; s_h = 1; s_w = 1
		fc2 = layer(fc1, k_h, k_w, c_i, c_o, s_h, s_w, padding="VALID", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize=False, wd=wd, steerable=False)
		tf.add_to_collection("activationMagnitudes", conv4_1)
		if useType=="train":
	...
	with tf.variable_scope("out", reuse=(not first)) as scope:
		...
		out_relu = layer(fc2, k_h, k_w, c_i, c_o, s_h, s_w, padding="VALID", first=first, useType=useType, resuse_batch_norm=resuse_batch_norm, normalize=False, wd=wd, steerable=False)
		out_relu = tf.reshape(out_relu, [batch_size,1,1,16,10])
		out = tf.reduce_max(out_relu, axis=-2)
		angles = [a*2*numpy.pi/16 for a in range(16)]
		angles_out = tf.argmax(out_relu, axis=-2)
		angles_out = tf.gather(angles, angles_out)
	...
	with tf.variable_scope("outAngles", reuse=(not first)) as scope:
		...
		fc2L = tf.unstack(tf.squeeze(tf.reshape(fc2, [batch_size, 1, 1, 16, 96])), axis=-1)
		resL = []
		for fc2Node in fc2L:
