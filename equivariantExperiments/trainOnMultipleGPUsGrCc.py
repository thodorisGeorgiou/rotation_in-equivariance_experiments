...
# baseDir = "trained_classification_complexWeights/grGrSumCcFCNet5L3x96_64_2x36_32_16_fs_9_all_7_16Bins_adjustedBN_weightMask_trainAll_dropout_3x0.5_0MAD_droppingLR_30_"+run+"_"
baseDir = "trained_classification_complexWeights/grSumCcNet5L3x96_64_2x36_32_16_fs_9_all_7_16Bins_adjustedBN_weightMask_trainAll_0MAD_droppingLR_30_"+run+"_"
...
if __name__ == '__main__':
	...
	mnsitTrain = {'x': numpy.load("/scratch/georgioutk/mnist/my_mnist_rot/mnist_rot_train_imanges.npy"), 'y': (numpy.load("/scratch/georgioutk/mnist/my_mnist_rot/mnist_rot_train_labels.npy")[:,:1]).astype(numpy.int32)}
	mnsitTest = {'x': numpy.load("/scratch/georgioutk/mnist/my_mnist_rot/mnist_rot_test_imanges.npy"), 'y': (numpy.load("/scratch/georgioutk/mnist/my_mnist_rot/mnist_rot_test_labels.npy")[:,:1]).astype(numpy.int32)}
	...
	[next_example, next_label], [next_testExample, next_testLabel], [trainIterator, testIterator] = preprocessing.colorInput(batch_size, mnsitTrain['x'], mnsitTrain['y'], testBatch_size=testBatch_size)
	# [next_example, next_label], [next_testExample, next_testLabel], [trainIterator, testIterator] = preprocessing.gradientOrientation(batch_size, mnsitTrain['x'], mnsitTrain['y'], testBatch_size=testBatch_size)
	...
	for step in range(int(numEpochs*mnsitTrain['x'].shape[0]/batch_size)):
		...
		if step % (mnsitTrain['x'].shape[0]//(batch_size*0.25)) == 0 and step != 0:
			...
			valExEntropy, SuccRateValidation = testNetwork(sess, [top_1, testXEntropy], testBatch_size, testIterator, mnsitTrain['x'], mnsitTrain['y'], log)
			...
			TestExEntropy, SuccRateTest = testNetwork(sess, [top_1, testXEntropy], testBatch_size, testIterator, mnsitTest['x'], mnsitTest['y'], log)
